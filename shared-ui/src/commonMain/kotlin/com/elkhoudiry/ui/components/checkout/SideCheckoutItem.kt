...
fun SideCheckoutItem(
    modifier: Modifier, items: List<ListingItem>
) {
    Surface(
        modifier = modifier, shape = AppTheme.shapes.small, elevation = AppTheme.dimens.checkoutItemElevation.dp
    ) {
        Column(
            ...
            Text(
                text = items[0].name, color = AppTheme.colors.onSurface.toColor(), style = AppTheme.type.body1
            )
            Text(
