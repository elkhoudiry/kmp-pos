...
fun SideCheckoutUI(modifier: Modifier, state: CheckoutState, onEvent: (CheckoutEvent) -> Unit) {
    ...
    Column(
        ...
        LazyColumn(modifier = Modifier.weight(6f).simpleVerticalScrollbar(lazyState), state = lazyState) {
            itemsIndexed(state.items().keys.toList()) { index, item ->
                SideCheckoutItem(
                    modifier = Modifier.height(AppTheme.dimens.checkoutItemHeight.dp)
                        .padding(AppTheme.dimens.checkoutItemPadding.dp)
                        .combinedClickable(
                            onClick = { onEvent(CheckoutEvent.ItemClick(state.items()[item]!![0])) },
                            onLongClick = {  }
                        ),
        ...
        Column(
            ...
            Text(
                "${getMRString(MR.strings.total, CommonGlobals.resContext)}: ${state.totalPrice()} $",
            ...
            Row {
                OutlinedButton(modifier = Modifier.height(AppTheme.dimens.checkoutClearButtonHeight.dp),
                ...
                Button(modifier = Modifier.weight(5f).height(AppTheme.dimens.checkoutClearButtonHeight.dp),
